<template>
  <el-dialog v-model="display" @close="close" title="申请收录新codeforces账户" ref="formRef">
    <el-form :model="data" :rules="rules" ref="formRef" label-width="100px">
      <el-form-item label="用户名" prop="codeforces_id">
        <el-input v-model="data.codeforces_id" placeholder="请输入用户名"></el-input>
      </el-form-item>
...
let submit = ()=> {
  ...
  formRef.value.validate((valid) => {
    if (valid) {
      loading.value = true
      axiosUtil.codeforcesAPI.addNewCodeforcesAccount(data.value.codeforces_id,parseInt(data.value.is_main)).then(res=>{
