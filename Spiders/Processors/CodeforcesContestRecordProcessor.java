...
public class CodeforcesContestRecordProcessor implements IProcessor{
    @Override
    public void process(Task task) {
        ...
        try{
            if (main.getString("status").equals("OK")){
                JSONArray contestRecordList = main.getJSONArray("result"); // 提取result字段
                JSONObject result = new JSONObject();
                result.put("contestRecordList",contestRecordList);
                String url = task.getRequestParams().getUrl();
                result.put("handler",url.split("=")[1]); //在这里提前获取用户名，方便之后操作
                Map<String,Object> res = new HashMap<>();
                res.put("handler","codeforcesContestRecordResultHandler");//指定Dispatcher的handler，与指定Dispatcher相似,首字母小写CodeforcesRatingChangeContestResultHandler变成codeforcesRatingChangeContestResultHandler
                res.put("result",result);//爬取的结果放到result中
